...
1. **v-model**: 主要用于表单元素的双向数据绑定。

```vue
<input type="text" v-model="message">
```

1. **v-bind**: 绑定 HTML 属性，简写为 `:属性名`。

```vue
<img :src="imageSrc" :alt="imageAlt">
```

1. **v-on**: 绑定事件监听器，简写为 `@事件名`。
...
1. **v-pre**: 跳过编译当前元素和它的子元素。

```vue
<div v-pre>{{ This will not be compiled }}</div>
```

1. **v-cloak**: 防止模板闪烁，直到 Vue 实例完全编译。

```vue
<div v-cloak>{{ message }}</div>
```

1. **v-text**: 绑定纯文本，等同于插值表达式 `{{}}`。
...
1. **beforeRouteEnter**：进入组件之前调用，无法获取组件实例（`this`），因为组件还未被创建。
...
2. **beforeRouteLeave**：离开组件前调用，通常用于确认离开前的操作。
...
3. **beforeRouteUpdate**：路由参数变化时调用，例如动态路由时。
...
1. **history 模式**：使用浏览器的 `history.pushState` API，无需带 `#` 号。
...
2. **hash 模式**：使用 URL 的哈希 (`#`) 作为路径片段，适合无需服务器支持的项目。
...
1. **使用虚拟环境** 管理依赖。
2. **利用 SQLAlchemy** 进行数据库操作。
3. **结合 Pydantic 模型** 验证输入数据。
4. **通过状态码和 HTTPException** 返回适当的响应。
5. **结合中间件**，如 `CORSMiddleware` 来处理跨域问题。
...
2. **Gunicorn 配置 (gunicorn_conf.py)**
