...
相邻兄弟选择器：ul li ~ li { ... } （选择ul li后相邻的li标签）
...
新建一个清除浮动的盒子：clear: both;

大盒子设置：overflow: hidden;
...
align-self：用于单个元素的对齐方式，覆盖容器的align-items设置。
...
扩展比例：flex-grow: number; （值越大，元素扩展越大）

收缩比例：flex-shrink: number; （值越大，元素收缩越多）

基础大小：flex-basis: 基准值; （设置元素初始大小）
...
取消input点击效果：outline: none;

取消input默认样式：-webkit-appearance: none;
...
function
原型对象（prototype）
JavaScript 是一种基于原型的编程语言，所有的对象都是通过原型链进行继承的。prototype 是每个构造函数都自带的一个属性，允许其实例对象共享属性和方法。
...
原型的作用
属性和方法共享：挂载在 prototype 上的属性和方法可以被该构造函数的所有实例共享。

减少内存占用：由于所有实例共享 prototype 上的方法，避免每个实例都创建重复的函数。
...
总结
构造函数可以创建对象，并通过 this 赋予新对象属性。

构造函数的原型对象（prototype）可以存放方法，供所有通过该构造函数创建的实例使用。

属性通常用于记录数据，方法用于处理业务逻辑，并挂载在 prototype 上供实例共享。
...
原型链
原型链是由构造函数的 prototype 和实例对象的 __proto__ 组成的链式结构，构成了 JavaScript 中的继承机制。
...
search()：用于查找字符串中与正则表达式匹配的部分，返回匹配的起始位置。
...
3. 配置 Webpack
创建 Webpack 配置文件： 在项目根目录创建 webpack.config.js 文件：
...
创建入口文件： 在根目录下创建 src/index.js，并在其中编写一些示例代码。
...
webpack --config webpack.config.js
4. 处理 CSS 和预处理器
处理 CSS 文件： 安装 css-loader 和 style-loader：


npm install css-loader style-loader --save-dev
处理 Less 文件： 安装 less-loader 和依赖：


npm install css-loader style-loader less less-loader --save-dev
处理 Sass 文件： 安装 sass-loader 和依赖：
...
Ajax
Ajax 是一种用于在不重新加载整个网页的情况下从服务器获取数据的技术，通常使用 XMLHttpRequest 对象来执行。
...
$("#email").blur(function () {
    ...
    // 请求URL，传递参数给服务器
    let url = 'http://localhost/checkEmail.php?email=' + email;
​
    // 初始化请求
...
Ajax 中常用属性与方法：
open(method, url, async): 初始化请求，其中 method 是 GET 或 POST，url 是请求地址，async 表示是否异步。
...
POST 请求示例
POST 请求通常用于提交表单数据。在 POST 请求中，数据通常作为请求体发送，需要设置请求头。
...
​
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                var response = JSON.parse(xhr.responseText); // 将JSON字符串转换为对象
                if (response.code == 0) {
                    alert('登录成功！');
                    location.href = 'http://localhost/profile.php';
                } else {
                    alert('登录失败，用户名或密码错误');
                }
            }
...
JSON 解析
从服务器返回的 JSON 数据可以使用 JSON.parse() 进行解析，转化为 JavaScript 对象。

var obj = JSON.parse(xhr.responseText);
JSONP 技术
JSONP 允许你跨域请求数据，服务器返回的是一个函数调用，而不是纯粹的 JSON 数据。

echo $callbackFunction . '(' . json_encode($data) . ')';
在客户端 JavaScript 中：
...
总结
GET 请求通过 URL 传递参数，适合简单请求。

POST 请求通过请求体传递参数，适合提交表单等复杂请求。

JSON 是常用的数据格式，JSON.parse() 用于将字符串转换为对象。

JSONP 允许跨域请求，但仅限于 GET 请求。
...
module.exports = {
  plugins: {
    'postcss-pxtorem': {
      rootValue: 37.5, // 根据设计稿设置，375px 设计稿根元素为 37.5
      propList: ['*'], // 转换所有属性
    },
  },
};
...
Vue 的生命周期
1. beforeCreate: 实例初始化之后、数据观测 (data) 和事件监听 (methods) 之前调用。此时实例对象中的 data 和 methods 尚未被创建，DOM 还不可见。
2. created: 实例初始化完成，data 已经可用，但 DOM 尚未生成。在这里可以进行数据请求 (AJAX) 或操作数据。
3. beforeMount: 在挂载开始之前调用。此时模板已经被编译为 render 函数，但尚未将其渲染为实际的 DOM 节点。常用于预处理逻辑。
4. mounted: 实例被挂载到 DOM 上后调用。此时已经可以访问 DOM 元素，常用于操作 DOM 或触发某些视图初始化逻辑。该钩子只调用一次。
5. beforeUpdate: 当响应式数据发生变化时，视图更新之前调用。可以在这里对即将更新的状态进行处理。
6. updated: 视图更新完成后调用。此时 DOM 已经更新，可以执行与 DOM 相关的操作。
7. beforeDestroy: 实例销毁之前调用。此时实例还未完全被销毁，仍然能使用 data 和 methods 等，但常用于清理侦听器、定时器等。
8. destroyed: 实例销毁之后调用。此时所有的事件监听和子实例已经被移除，所有的资源都可以释放。
Vue 生命周期设计模式：MVVM
M (Model): 数据模型，主要用于定义和存储数据。

V (View): 视图层，页面展示数据。

VM (ViewModel): 视图模型层，负责将数据与视图进行绑定和同步。

Vue 的核心设计是通过数据驱动视图更新，使用 Vue 实现数据和视图的双向绑定，使开发者只需关心业务逻辑，减少了操作 DOM 的代码。

Vue 自定义指令
Vue 允许开发者注册自定义指令用于操作 DOM 元素。自定义指令分为全局注册和局部注册。
...
<input v-focus>
Vue 指令
Vue 的指令是 Vue.js 模板语法的核心，用于对 DOM 元素执行动态操作。指令以 v- 开头，如 v-model, v-if，并支持简写形式。
...
v-bind: 绑定 HTML 属性，简写为 :属性名。

<img :src="imageSrc" :alt="imageAlt">
v-on: 绑定事件监听器，简写为 @事件名。
...
<div v-else-if="otherCondition">Else If block</div>
<div v-else>Else block</div>
v-show: 控制元素的显示/隐藏。

<div v-show="isVisible">This will be shown or hidden</div>
v-pre: 跳过编译当前元素和它的子元素。

<div v-pre>{{ This will not be compiled }}</div>
v-cloak: 防止模板闪烁，直到 Vue 实例完全编译。

<div v-cloak>{{ message }}</div>
v-text: 绑定纯文本，等同于插值表达式 {{}}。

<div v-text="message"></div>
v-html: 绑定 HTML 内容。

<div v-html="htmlContent"></div>
v-once: 只渲染一次，后续数据改变时不会重新渲染。

<div v-once>{{ message }}</div>
Vue 属性详解
1. 属性计算 (computed)
computed 属性是基于其依赖进行缓存的，只有当依赖的值发生变化时才会重新计算。这使得 computed 非常适合处理涉及多个属性或需要复杂计算的场景。
...
computed 计算属性类似于 methods，但不同的是它们有缓存功能。当依赖的值不变时，多次访问计算属性不会重新执行函数。

2. 属性侦听 (watch)
watch 用于观察和响应数据的变化，适用于需要在数据变化时执行异步或开销较大的操作（如 AJAX 请求）时。

<div id="container">
    <h1>{{ message }}</h1>
    <button @click="sub">点击按钮减掉一个字符</button>
...
watch 是属性侦听器，用于观察某个数据的变化并执行对应的逻辑操作。

3. 过滤器 (filters)
filters 用于对数据进行格式化处理，常用于模板中数据展示的修饰。
...
new Vue({
    ...
    filters: {
        getSex(str) {
            return str.includes("红色") ? "她可能是女生" : "未知";
        }
    }
});
</script>
filters 是用来格式化输出数据的。你可以在模板中用 | 管道符来使用过滤器。

Vue 事件修饰符
事件修饰符可以帮助我们更加灵活地控制事件处理的行为，以下是常见的事件修饰符以及它们的使用方法：

<!-- 修饰符用法 -->
<button @click.stop="doSomething">阻止事件冒泡</button>
<button @submit.prevent="onSubmit">阻止默认表单提交行为</button>
<button @click.once="doOnce">只执行一次</button>
<button @scroll.passive="onScroll">提高滚动事件性能</button>
<button @click.self="onClickSelf">点击自身触发</button>
常用事件修饰符：
.stop: 阻止事件冒泡。

.prevent: 阻止浏览器的默认行为。

.capture: 事件在捕获阶段触发，而不是冒泡阶段。

.self: 只有在事件由当前元素本身触发时才执行。

.once: 事件只触发一次。

.passive: 用于提高性能，通常用于 scroll 等事件。

组合键修饰符：

<input @keydown.ctrl.enter="submit"> <!-- 按下 Ctrl + Enter 时触发 -->
捕获事件与事件冒泡示例：
<!-- 阻止事件冒泡 -->
<div @click="divClick">
  <ul @click.stop="ulClick">
    <li @click.stop="liClick">点击 li</li>
  </ul>
</div>
​
<!-- 捕获事件 -->
<div @click.capture="divClickCapture">
  <ul @click.capture="ulClickCapture">
    <li @click.capture="liClickCapture">捕获 li</li>
  </ul>
</div>
Vue 插槽 (Slot)
插槽允许父组件向子组件的特定位置插入内容。Vue 提供了两种类型的插槽：默认插槽和具名插槽。
...
3. 作用域插槽
作用域插槽允许子组件将数据传递回父组件。父组件可以通过具名插槽接收子组件传递的值，并在模板中使用。

<!-- 父组件中 -->
<my-box>
  <template #c="slotProps">
    <div>{{ slotProps.user }}</div>
    <div>{{ slotProps.animal }}</div>
  </template>
</my-box>
​
<!-- 子组件中 -->
<template>
  <div>
    <slot name="c" :user="userData" :animal="animalData"></slot>
  </div>
</template>
​
<script>
...
Vue CLI 脚手架
Vue CLI 是一个官方提供的标准化开发工具，方便我们快速创建和管理 Vue 项目。通过 Vue CLI，你可以轻松地创建项目、配置开发环境、管理依赖等。
...
在创建项目时，你可以选择默认配置或者自定义配置。默认配置通常已经包括 Vue Router 和 Vuex，适合大部分项目需求。

Vue 脚手架项目结构
my-project
├── node_modules      # 项目依赖的第三方库
├── public            # 公共文件目录，项目的根 HTML 文件存放在这里
│   ├── favicon.ico   # 浏览器标签的图标
│   └── index.html    # 项目的主 HTML 文件
├── src               # 主要开发目录
│   ├── assets        # 静态资源文件，如图片、视频等
│   ├── components    # 公共组件目录
│   ├── router        # 路由配置目录
│   ├── views         # 路由对应的页面组件
│   ├── App.vue       # 根组件
...
├── babel.config.js   # Babel 配置文件，将 ES6 转换为 ES5
├── jsconfig.json     # 配置路径别名、目标 JS 输出等
├── package.json      # 项目描述文件，列出项目依赖、脚本等
├── README.md         # 项目说明文件
└── vue.config.js     # Vue CLI 配置文件
Vue 路由配置
Vue Router 是 Vue.js 的官方路由管理器，它用于配置单页面应用中的导航。

声明式导航
使用 <router-link> 声明式导航链接：
...
// 定义路由：/home
this.$router.push({ name: 'home', query: { page: 1, limit: 10 } });
返回上一级
// 返回上一个页面
this.$router.go(-1);
路由守卫
路由守卫用于拦截导航，决定是否允许导航继续、取消或重定向到其他路径。
...
组件内守卫
beforeRouteEnter：进入组件之前调用，无法获取组件实例（this），因为组件还未被创建。
...
beforeRouteLeave：离开组件前调用，通常用于确认离开前的操作。
...
beforeRouteUpdate：路由参数变化时调用，例如动态路由时。
...
路由元信息 meta
元信息可以为路由添加自定义数据，例如是否需要登录等。可以通过 meta 属性进行配置。
...
history 模式：使用浏览器的 history.pushState API，无需带 # 号。

const router = new VueRouter({
  mode: 'history',
  routes
});
hash 模式：使用 URL 的哈希 (#) 作为路径片段，适合无需服务器支持的项目。
...
缓存组件 - keep-alive
keep-alive 是 Vue 提供的内置组件，用于缓存动态组件，避免重复渲染。

<keep-alive>
  <component :is="currentView"></component>
</keep-alive>
include 和 exclude
可以通过 include 和 exclude 属性控制哪些组件应该被缓存或排除。
...
Vuex
Vuex 的五大核心理念
State：存储应用的全局状态，类似于 Vue 组件中的 data。

Mutations：用于同步修改 state 的方法，类似于 Vue 组件中的 methods（不处理异步操作）。

Actions：用于提交 mutations，可以包含异步操作，类似于 Vue 组件中的 methods（处理异步操作）。

Getters：用于计算和过滤 state 中的数据，类似于 Vue 组件中的 computed。

Modules：用于将 state、mutations、actions、getters 分模块管理，适合大型项目。
...
uni.navigateTo：保留当前页面，跳转到新页面，可以使用 uni.navigateBack 返回。

uni.navigateTo({
  url: './index/index'
});
uni.redirectTo：关闭当前页面并跳转到新页面。

uni.redirectTo({
  url: './index/index'
});
uni.reLaunch：关闭所有页面并打开新页面。

uni.reLaunch({
  url: './index/index'
});
uni.switchTab：跳转到 tabBar 页面并关闭其他非 tabBar 页面。
...
6. Nuxt.js 部署到服务器
安装 PM2 管理器：在宝塔面板中下载并安装 PM2 管理器。
...
设置反向代理：设置 Nginx 反向代理，指向 Nuxt.js 监听的端口（例如 127.0.0.1:3003）。
...
打包应用：运行 npm run build 打包应用，打包后上传以下文件至服务器：
...
GLTF/GLB (GL Transmission Format/Binary)

X3D (XML-based 3D model format)
...
.autoClearColor: 是否清除颜色缓存，默认为 true。

.autoClearDepth: 是否清除深度缓存，默认为 true。

.autoClearStencil: 是否清除模板缓存，默认为 true。

.domElement: 返回画布节点，需要手动插入到 HTML 中。

.shadowMap.enabled: 是否允许在场景中使用阴影贴图，默认 false。

.shadowMap.type: 定义阴影贴图类型，支持 THREE.PCFShadowMap（默认）等。
...
核心步骤：
Raycaster：创建射线投射器，用于从鼠标指向生成射线。

Vector2：用于表示鼠标的二维坐标。

setFromCamera：通过鼠标位置和摄像机生成射线。

intersectObjects：计算射线与场景中所有物体的交点，返回交点对象数组。

响应交点：点击的物体可以进行颜色变化或执行其他逻辑。
...
核心步骤：
UV 坐标：指定 3D 物体顶点上的纹理坐标。

BufferAttribute：创建并应用 UV 属性，该属性指定了每个顶点的纹理坐标。
...
核心步骤：
computeVertexNormals()：自动计算几何体的每个顶点的法向量，用于实现光照效果。
...
python
FastAPI 是一个现代、快速的 Web 框架，SQLAlchemy 是一个 ORM（对象关系映射）库，可以与数据库进行交互。以下是 FastAPI 与 SQLAlchemy 的基本用法，以及如何处理 HTTP 请求和响应。
...
SQLAlchemy 主要用来执行 CRUD（创建、读取、更新、删除）操作，以下是基本的 SQLAlchemy 查询方法：

# 查询操作
session.query(Model).filter(Model.id == 1).first()  # 查找 ID 为 1 的记录
session.query(Model).all()  # 返回所有记录
session.query(Model).order_by(Model.name).all()  # 按名称排序
session.query(Model).count()  # 获取记录数
创建 Python 虚拟环境
为了隔离项目依赖，可以使用虚拟环境：
...
async def login(user: User, response: Response):
    if user.username == "test":
        response.status_code = status.HTTP_202_ACCEPTED  # 修改状态码为 202
        return {"message": "Login accepted"}
    return {"message": "Invalid username"}
异常处理
通过 HTTPException 返回自定义 HTTP 错误响应：
...
​
        if self.strength_level == 4:
            self.msg = "密码强度合格"
        return self.msg
结合密码工具类进行注册
@app.post("/register")
async def register(user: User):
    password_tool = PasswordTool(user.password)
    validation_message = password_tool.validate()
    if validation_message == "密码强度合格":
        # 假设已经读取和写入用户数据
...
利用 SQLAlchemy 进行数据库操作。

结合 Pydantic 模型 验证输入数据。

通过状态码和 HTTPException 返回适当的响应。

结合中间件，如 CORSMiddleware 来处理跨域问题。

服务器配置和部署指南
上传文件到服务器
使用 VSCode SFTP 插件

安装 SFTP 插件，按 Ctrl + Shift + P 调出命令行。

运行命令 SFTP:config，生成 .vscode/sftp.json 配置文件。

配置完成后，可以通过 VSCode 直接上传或下载文件至服务器。
...
将 FastAPI 应用与 Nginx 反向代理配置在一起。

server {
    listen 8088;
    server_name 47.96.28.91;  # 服务器 IP 或域名

    location / {
        proxy_pass http://127.0.0.1:8099;  # FastAPI 应用地址
...
通过这种方式，你可以将 FastAPI 后端和 Vue 前端通过 Nginx 部署在同一台服务器上，并且通过代理管理前后端交互。
